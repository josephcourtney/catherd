...
Human display normalization must not mutate machine-readable values. Multiline/irregular whitespace may be collapsed in human views, but `show` must wrap long command and path values rather than discard them. The TUI tree may abbreviate values for scanning only when selecting the object exposes the complete display-normalized value in the inspector. `show --json` and `inspect` retain exact underlying values.
...
The inspector contains richer information for the currently selected object and is the lossless human-readable escape hatch for compact tree rows. Command-like titles may be normalized for identity/display, while complete display-normalized paths, process commands, history commands, and identifiers remain available in the inspector.
